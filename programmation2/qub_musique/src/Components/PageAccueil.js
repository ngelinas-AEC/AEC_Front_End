...
export class Accueil extends React.Component  {
    ...
    affichageCataloque(){
        ...
        return ( <Container >
                    {this.header()}
                    {this.afficherEntete()}
                    <Catalogue handleAfficheEntete={() => this.setState({accueil: true, recherche:""})} recherche={this.state.recherche} onClick={() => this.setState({ accueil: false})}/>
                </Container>
                );
        } else {
            return <Connexion handleStatutConnexion={(connect) => this.setState({ connected: connect })} />;
        }
    }
